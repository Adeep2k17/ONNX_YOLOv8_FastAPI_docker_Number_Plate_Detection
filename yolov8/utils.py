...
# custom model trained on class
class_names = ['vehicle_num_plate']

# Create a list of colors for each class where each color is a tuple of 3 integer values
rng = np.random.default_rng(3)
colors = rng.uniform(0, 255, size=(len(class_names), 3)).tolist()


def nms(boxes: np.ndarray, scores: np.ndarray, iou_threshold: float)-> list[Any]:
    # Sort by score (Non maximum suppression (NMS))
...
def multiclass_nms(boxes: np.ndarray, scores: np.ndarray, class_ids: np.ndarray, iou_threshold: float)-> list[Any]:
